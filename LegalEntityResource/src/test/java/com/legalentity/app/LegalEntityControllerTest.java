...
class LegalEntityControllerTest {
	...
	@Test
	@Order(1)
	// http://localhost:8081/createlegalentities/ URL to be called and just check
	// the response
	...
	@Order(2)
	/*
	 * http://localhost:8081/getAlllegalentities/ URL to be called and check if the
	 * first element's entity name is "Entity1" or not
	 */
	public void getAllLegalEntities() throws Exception {
		final String uri = "/getAlllegalentities";

		this.mockMvc.perform(get(uri)).andExpect(status().isOk()).andExpect(content().contentType(JSON_TYPE))
				.andExpect(jsonPath("$[0].entityName", is("Entity1")));
	}

	/*
	 * http://localhost:8081//updatelegalentity/{entityname} URL with entityName as
	 * "Entity1" and pass a new object with the countryOfIncorp as JPN and check the
	 * output is updated with the value contains JPN
	 */
	@Test
	@Order(3)
	...
	/*
	 * Check the URL http://localhost:8081/addlegalentity/ and pass the object
	 * (Entity4) in POST request and check if the response contains "Entity4" ,
	 * which is added just now
	 */
	@Test
	@Order(4)
	public void addEntity() throws Exception {
		...
		LegalEntityShareHolderBean entity1SH1 = new LegalEntityShareHolderBean("Entity1ShareHolder1", "US", 5);
		LegalEntityShareHolderBean entity1SH2 = new LegalEntityShareHolderBean("Entity1ShareHolder2", "US", 5);
		LegalEntityBean legalEntity = new LegalEntityBean("Entity4", new Date(), "US", 10,
				new ArrayList<>(Arrays.asList(entity1SH1, entity1SH2)));

		MvcResult mvcResult = this.mockMvc
				.perform(post(uri).contentType(JSON_TYPE).content(convertObjectToJsonBytes(legalEntity))).andDo(print())
				.andReturn();
	...
	/*
	 * http://localhost:8081//updatelegalentity/{entityname} URL in our case , we
	 * remove Entity2 and check if the value is still there in the list of objects
	 */
	@Test
	@Order(5)
